...
def main(args):
    ...
    target_img = jnp.zeros((args.height, args.width, 3))
    ...
    def save_ckpt_data(i_iter):
        ...
        next_state, vid = rollout_fn(nca_params, _rng, state)
        print(vid.shape)
    ...
    for i_iter in range(args.n_iters):
        train_state, losses_i = jax.lax.scan(train_step, train_state, split(rng, args.n_iters_chunk))
        ...
        if i_iter % (args.n_iters // 5) == 0:
            save_ckpt_data(i_iter)
    save_ckpt_data(args.n_iters)
