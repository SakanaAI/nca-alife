...
def tf_consistent_bilinear_upsample(imgs, scale_factor=1.0):
    ...
    output = torch.nn.functional.grid_sample(imgs, grid, mode='bilinear', padding_mode='zeros', align_corners=True)
...
"""
adapted from: https://github.com/IVRL/DyNCA/blob/main/models/MSOEmultiscale.py
Changes:
- below code
- and set align_corners=True (search for it above)
"""
...
class MyOpticalFlowNet():
    ...
    def get_optical_flow(self, x1, x2):
        """
        x1, x2: torch.Tensor of shape (B, 3, H, W) and values in [0, 1]
        """
